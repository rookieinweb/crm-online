...
export async function mockGetDashboard() {
  ...
  const todos = customers
    .filter((item) => item.nextFollowAt && item.status !== 'lost')
    .sort((a, b) => new Date(a.nextFollowAt) - new Date(b.nextFollowAt))
    .slice(0, 5)
    .map((item) => ({
      id: `todo_${item.id}`,
      customerId: item.id,
      ...
      priority: item.nextFollowAt?.startsWith(today) ? 'high' : 'normal'
    }))
  ...
  return {
    stats: {
      ...
      todayFollowCount,
      todayVisitCount: visitTasks.filter((item) => item.planAt?.startsWith(today)).length,
      dealAmount
    },
    todos,
    visitTasks: visitTasks.filter((item) => item.planAt?.startsWith(today))
  }
}

export async function mockGetCustomers(params = {}) {
  await delay()
  let list = [...customers]
...
export async function mockGetCustomer(id) {
  await delay()
...
export async function mockCreateCustomer(data) {
  await delay()
  const item = {
    id: genId('c'),
    ownerId: 'u001',
...
export async function mockUpdateCustomer(id, data) {
  await delay()
...
export async function mockGetFollowRecords() {
  await delay()
...
export async function mockGetVisitTasks() {
  await delay()
  return {
    tasks: visitTasks.filter((item) => item.planAt?.startsWith(today)),
    records: visitRecords.slice().sort((a, b) => new Date(b.checkinAt) - new Date(a.checkinAt))
  }
}

export async function mockCheckin(taskId, checkinData = {}) {
  await delay()
  const task = visitTasks.find((item) => item.id === taskId)
  if (!task) throw new Error('拜访任务不存在')
  task.status = 'done'
...
export { customers, followRecords, visitTasks, visitRecords }
